...
use crate::{parse_number, ParseResult, Parseable, Parsed, Puzzle, Solver};
...
impl<'a> Parsed<&'a str> for Lookup {
    fn parse_wyz(text: &str) -> ParseResult<&str, Lookup> {
        let (text, seeds) =
            preceded(tag("seeds:"), many1(preceded(space1, parse_number::<i64>)))(text)
                .tap_err(|err| tracing::error!(?err, "could not parse the seed list"))?;
        let (text, almanac) = text
            .trim_start()
            .parse_wyz()
            .tap_err(|err| tracing::error!(?err, "could not parse an almanac"))?;
        Ok((
            text,
            Self {
                seeds,
            ...
            .min_location(
                ...
                self.seed_ranges
                    ...
                    .inspect(|Range { start, end }| {
                        tracing::debug!(bgn = ?start, ?end, len = end - start)
                    })
...
impl Almanac {
    ...
    pub fn min_location(&self, seeds: impl IntoIterator<Item = i64>) -> Option<i64> {
        seeds
            .into_iter()
...
impl<'a> Parsed<&'a str> for Almanac {
    fn parse_wyz(text: &'a str) -> ParseResult<&'a str, Self> {
        ...
        let (text, seed_soil) =
            preceded(header("seed", "soil"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse seed/soil map"))?;
        let (text, soil_fertilizer) =
            preceded(header("soil", "fertilizer"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse soil/fertilizer map"))?;
        let (text, fertilizer_water) =
            preceded(header("fertilizer", "water"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse fertilizer/water map"))?;
        let (text, water_light) =
            preceded(header("water", "light"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse water/light map"))?;
        let (text, light_temperature) =
            preceded(header("light", "temperature"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse light/temperature map"))?;
        let (text, temperature_humidity) = preceded(
            header("temperature", "humidity"),
            many1(Relation::parse_wyz),
        )(text.trim_start())
        .tap_err(|err| tracing::error!(?err, "could not parse temperature/humidity map"))?;
        let (text, humidity_location) =
            preceded(header("humidity", "location"), many1(Relation::parse_wyz))(text.trim_start())
                .tap_err(|err| tracing::error!(?err, "could not parse humidity/location map"))?;
        Ok((
            text,
            Self {
                seed_soil,
