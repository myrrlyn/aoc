use crate::{parse_number, prelude::*};
use nom::{
    bytes::complete::tag,
...
static ITEM: Solver = Solver::new(2023, 3, |t| t.parse_dyn_puzzle::<Blueprint>());
            ...
            for &Token { bgn, end, val } in part_nums {
                ...
                for sym_line in part_line.saturating_sub(1).max(0)..=(part_line + 1) {
                    ...
                    for symbol in self
                        ...
                        .unwrap_or_default()
                    {
                        // If the symbol is in the region `[.NUMBER.]`, save the number
            ...
            for &Token { bgn, end, val } in part_nums {
                ...
                for sym_line in part_line.saturating_sub(1).max(0)..=(part_line + 1) {
                    ...
                    for symbol in self
                        ...
                        .unwrap_or_default()
                    {
                        // If the symbol is in the region `[.NUMBER.]`, it's a part
                        if symbol.bgn >= bgn.saturating_sub(1).max(0) && symbol.bgn <= end {
                            // And furthermore, if the symbol is a gear, save its number.
                            if symbol.val == '*' {
                                self.gears
