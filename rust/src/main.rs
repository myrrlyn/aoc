...
impl Args {
    fn execute_program(&self) -> anyhow::Result<()> {
        ...
        let make_solver = wyz_aoc::solutions()
            ...
            .get(&day)
            .with_context(|| anyhow::anyhow!("{year}-{day:0>2} has no registered solution"))?;
		...
		// after the source text is destroyed.
            (make_solver)(source_text.as_str()).map_err(|err| anyhow::anyhow!("{err}"))?;
        ...
        if self.step != Step::Two {
            ...
            solver
                .prepare_1()
                .with_context(|| format!("error preparing {year}-{day:0>2}#1"))?;
            tracing::info!(step = 1, "running");
            solver
                .part_1()
                .with_context(|| format!("failure running {year}-{day:0>2}#1"))?
                .tap(|answer| tracing::info!(?year, ?day, part = 1, ?answer, "solved!"));
        }
        if self.step != Step::One {
            tracing::info!(step = 2, "preparing");
            solver
                .prepare_2()
                .with_context(|| format!("error preparing {year}-{day:0>2}#2"))?;
            tracing::info!(step = 2, "running");
            solver
                .part_2()
                .with_context(|| format!("failure running {year}-{day:0>2}#2"))?
                .tap(|answer| tracing::info!(?year, ?day, part = 2, ?answer, "solved!"));
        }
