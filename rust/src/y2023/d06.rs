    ...
    fn execute(&self) -> anyhow::Result<i64> {
        self.races
            ...
            .map(move |&Race { time, dist }| {
                (1..time)
                    // .inspect(|speed| tracing::debug!(?speed, ?time, ?dist, "computing my distance"))
                    .map(move |speed| speed * (time - speed))
                    // .inspect(|distance| tracing::debug!(?distance, "my distance"))
...
impl<'a> Parsed<&'a str> for Races {
    fn parse_wyz(text: &'a str) -> ParseResult<&'a str, Self> {
        let (text, times) =
            delimited(tag("Time:"), many1(preceded(space1, parse_number)), newline)(text)
        ...
        .tap_err(|err| tracing::error!(?err, "could not parse race distances"))?;
        let races = times
            .into_iter()
            .zip(dists.into_iter())
