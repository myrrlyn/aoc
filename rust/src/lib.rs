...
pub type DynParser = for<'a> fn(&'a str) -> ParseResult<&'a str, Box<dyn Puzzle>>;
...
pub fn solutions() -> &'static Registry {
    ...
    REGISTRY.get_or_init(|| {
        SOLVERS
            .iter()
            .fold(Registry::new(), |mut accum, &Solver { year, day, func }| {
...
pub trait Parsed<Input>: Sized {
    ...
    where
        Self: 'static + Puzzle,
    {
        Self::parse_wyz(src).map(|(rest, this)| (rest, Box::new(this) as Box<dyn Puzzle>))
    }
}
