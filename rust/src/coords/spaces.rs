use super::{Cartesian2DPoint, Cartesian3DPoint, Signed};
use std::{
...
impl<I: Signed, T> Cartesian2D<I, T> {
    ...
    pub fn contains(&self, Cartesian2DPoint { x, y }: Cartesian2DPoint<I>) -> bool {
        self.rows
            .get(&y)
    ...
    pub fn get(&self, Cartesian2DPoint { x, y }: Cartesian2DPoint<I>) -> Option<&T> {
    ...
    pub fn get_or_insert_with(
        ...
        self.bounds = match self.bounds.take() {
            ...
            Some((min, max)) => Some((point.min_unifying(min), point.max_unifying(max))),
        };
        out
    }
}
...
impl<I: Signed, T> Cartesian3D<I, T> {
    ...
    pub fn bounds_inclusive(&self) -> Option<(Cartesian3DPoint<I>, Cartesian3DPoint<I>)> {
        self.bounds
    }
    ...
    pub fn search_bfs<CS: IntoIterator<Item = Cartesian3DPoint<I>>>(
        ...
        mut searcher: impl FnMut(Cartesian3DPoint<I>, &Self, &mut VecDeque<Cartesian3DPoint<I>>),
    ) {
        ...
        RangeInclusive<I>: IntoIterator<Item = I>,
    {
        self.bounds.into_iter().flat_map(
            |(
                ...
                (z1..=z2)
                    .into_iter()
                    .flat_map(move |z| (y1..=y2).into_iter().map(move |y| (y, z)))
                    .flat_map(move |(y, z)| (x1..=x2).into_iter().map(move |x| (x, y, z)))
